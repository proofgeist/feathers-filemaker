...
const transformers = require('./fms/FMServerClient/transformersNew')
  ...
  expandQueryWithOperators (query){
    ...
    Object.keys(query).map((queryKey)=>{
      ...
      if(queryKey.startsWith('$')){
        newQ = newObject.assign(newQ , queryItem )// return with no change
        ...
        newQ[queryKey] = value, newQ[queryKey+'.op'] = this.fmOperator(operator)
      }
    })
    return newQ
